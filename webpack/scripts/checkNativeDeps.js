...
if (dependencies) {
	...
	try {
		// Check why dependency was installed, warn if something was wrongly installed.
		...
		if (filteredRootDependencies.length > 0) {
			...
			console.log(`
				...
				${chalk.bold(filteredRootDependencies.join(', '))} ${plural ? 'are native dependencies' : 'is a native dependency'} and should be installed inside of the "./release/app" folder. First, uninstall the packages from "./package.json":
		...
		console.log('An error occured while checking native dependencies.');
	}
}
